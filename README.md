...
### 🔌 Esquemas de Ligação
- Diagramas de cablagem com pinagem e conexões entre componentes
- Biblioteca de MCUs (Arduino, ESP32, Raspberry Pi, etc.)
- Biblioteca de componentes (sensores, displays, relés, etc.)
- Componentes custom com pinos configuráveis
- Cabos coloridos com roteamento automático
- Tabela de ligações e lista de material (BOM)
- Impressão em A4 com esquema, tabela e BOM
...
### 🏫 Gestão de Turmas (Mestre)
- Folha "Turmas" no Sheets Mestre: mapeia turma → URL do Sheets
- Login resolve URL automaticamente pela turma do aluno
- Professor sincroniza turmas com botão 📡 na lista de turmas
- URL manual oculto em ⚙️ Configuração Avançada (fallback)

### 📋 Tomada de Conhecimento
- Estado por UFCD: inativa / conhecimento / autoavaliação / ambas
- Links Google Drive para planificações e documentos por UFCD
- Aluno consulta critérios, links, e assina digitalmente
- Registo no Sheets: folha "Tomada Conhecimento" com timestamp
- Assinatura digital táctil (dedo no telemóvel, rato no PC)
- Campo Nº UFCD separado; tab nomeado como AA_UFCD{nº}
...
| `Nexus_ESECCinfaesCTE_Perfboard.html` | Perfboard Designer — projetar PCBs e breadboards | 102K |
| `Nexus_ESECCinfaesCTE_Esquemas.html` | Esquemas de Ligação — diagramas de cablagem | 70K |
| `Nexus_ESECCinfaesCTE_Manual_Perfboard.html` | Manual do Perfboard Designer | 15K |
| `Nexus_ESECCinfaesCTE_Manual_Esquemas.html` | Manual dos Esquemas de Ligação | 15K |
