...
var SH_USERS = "Utilizadores";
var SH_TURMAS = "Turmas";

// ── Security ─────────────────────────────────────────────────────────
// Change this token to something unique. Must match index.html, Professor.html, Aluno.html.
var API_TOKEN = "nexus_eac_2026_token";
var SH_LOG   = "Log Acessos";
...
var HDR_LOG = ["Timestamp","ID","Tipo","Turma","Ação","Detalhe"];
...
function doGet() {
  return jsonResp({
    ...
    actions: ["login", "ping", "getUsers", "addUser", "updateUser", "removeUser",
              ...
              "getTurmas", "saveTurma", "removeTurma"]
  });
}
...
function doPost(e) {
  ...
  try {
    ...
    switch(action) {
      ...
      case "getturmas":   result = handleGetTurmas(ss, payload);    break;
      case "saveturma":   result = handleSaveTurma(ss, payload);    break;
      case "removeturma": result = handleRemoveTurma(ss, payload);  break;
      default:            result = {ok: false, msg: "Ação desconhecida: " + action};
    }

    // Log the action (matches HDR_LOG: Timestamp, ID, Tipo, Turma, Ação, Detalhe)
    logSh.appendRow([
      timestamp(),
      payload.id || payload.uid || "—",
      requesterTipo,
      payload.turma || result.turma || "—",
      action,
      (result.msg || "ok") + " | " + JSON.stringify(payload).substring(0, 150)
    ]);
...
function handleLogin(ss, d) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    // Success! Resolve sheetsUrl: try Turmas sheet first, fallback to user row
    var userSheetsUrl = String(row[5]).trim();
    var turmaName = String(row[4]).trim();
    var resolvedUrl = userSheetsUrl;
    if (!resolvedUrl && turmaName) {
      var turmasSh = ss.getSheetByName(SH_TURMAS);
      if (turmasSh && turmasSh.getLastRow() >= 2) {
        var tData = turmasSh.getDataRange().getValues();
        for (var t = 1; t < tData.length; t++) {
          if (String(tData[t][0]).trim().toLowerCase() === turmaName.toLowerCase()) {
            resolvedUrl = String(tData[t][1]).trim();
            break;
          }
        }
      }
...
function handleGetUsers(ss, d) {
  ...
  var data = sh.getDataRange().getValues();
  var users = [];
  var filterTurma = d.turma ? String(d.turma).trim().toLowerCase() : null;
  for (var r = 1; r < data.length; r++) {
    var row = data[r];
    // Filter by turma if specified (case-insensitive, matches handleLogin)
    if (filterTurma && String(row[4]).trim().toLowerCase() !== filterTurma) continue;
    users.push({
...
function handleAddUser(ss, d) {
  ...
  var pwdHash = d.pwd ? (String(d.pwd).startsWith("h_") ? String(d.pwd) : hashPwd(String(d.pwd))) : hashPwd(id);
...
function handleUpdateUser(ss, d) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    if (d.pwd !== undefined)           sh.getRange(rowNum, 4).setValue(String(d.pwd).startsWith("h_") ? String(d.pwd) : hashPwd(String(d.pwd)));
...
// TURMAS
// ─────────────────────────────────────────────────────────────────────
function getOrCreateTurmas(ss) {
  var sh = ss.getSheetByName(SH_TURMAS);
  if (!sh) {
    sh = ss.insertSheet(SH_TURMAS);
    sh.appendRow(["Turma", "SheetsURL", "Ano Letivo", "Criado Em"]);
    sh.getRange(1, 1, 1, 4).setBackground("#0F2447").setFontColor("#fff").setFontWeight("bold");
...
function handleGetTurmas(ss, d) {
  var sh = ss.getSheetByName(SH_TURMAS);
  if (!sh || sh.getLastRow() < 2) return {ok: true, turmas: []};
  var data = sh.getDataRange().getValues();
  var turmas = [];
  for (var r = 1; r < data.length; r++) {
    turmas.push({
      turma:     String(data[r][0]).trim(),
      sheetsUrl: String(data[r][1]).trim(),
      anoLetivo: String(data[r][2]).trim()
    });
  }
  return {ok: true, turmas: turmas};
}

function handleSaveTurma(ss, d) {
  var nome = String(d.turma || "").trim();
  if (!nome) return {ok: false, msg: "Nome da turma obrigatório."};
  var sh = getOrCreateTurmas(ss);
  var data = sh.getDataRange().getValues();
  // Update if exists
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][0]).trim().toLowerCase() === nome.toLowerCase()) {
      if (d.sheetsUrl !== undefined) sh.getRange(r + 1, 2).setValue(d.sheetsUrl);
      if (d.anoLetivo !== undefined) sh.getRange(r + 1, 3).setValue(d.anoLetivo);
      return {ok: true, msg: "Turma '" + nome + "' atualizada."};
    }
  }
  // Insert new
  sh.appendRow([nome, d.sheetsUrl || "", d.anoLetivo || "", timestamp()]);
  return {ok: true, msg: "Turma '" + nome + "' registada."};
}

function handleRemoveTurma(ss, d) {
  var nome = String(d.turma || "").trim();
  if (!nome) return {ok: false, msg: "Nome da turma obrigatório."};
  var sh = ss.getSheetByName(SH_TURMAS);
  if (!sh || sh.getLastRow() < 2) return {ok: false, msg: "Turma não encontrada."};
  var data = sh.getDataRange().getValues();
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][0]).trim().toLowerCase() === nome.toLowerCase()) {
      sh.deleteRow(r + 1);
      return {ok: true, msg: "Turma '" + nome + "' removida."};
    }
  }
  return {ok: false, msg: "Turma não encontrada."};
}
