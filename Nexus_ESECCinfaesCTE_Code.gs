...
function doPost(e) {
  ...
  try {
    ...
    // _requesterTipo identifies WHO is making the request (aluno/prof).
    // It's separate from d.tipo (which may have other semantics like entrada/saida in presença).
    // Backward-compat: fall back to payload.tipo if _requesterTipo not provided.
    var tipo = String(payload._requesterTipo || payload.tipo || "prof").toLowerCase();
    var ALUNO_ACTIONS = ["vote","membervote","autoavsubmit","presencaregisto",
      "aularegisto","aulafeedback","presencafeedback","tomaconhecimento","getconfig","ping",
...
function doGet() {
  return jsonResp({ok:true, msg:"Nexus ESECCinfãesCTE v6 ativo. Use POST.", version:6,
    actions:["vote","membervote","profvote","profsummary","draw","summary","clearvotes","ping",
             ...
             "aulafeedback","presencafeedback","saveconfig","getconfig","tomaconhecimento","rfidaularegisto"]});
}
...
function handleAutoAvSubmit(ss, d) {
  ...
  // ── Resubmission: update in-place if same Nome + Nº UFCD + Ano/Turma exists ──
  // Key columns: B=Nome (idx 1), D=Nº UFCD (idx 3), E=Ano/Turma (idx 4)
  var keyNome = String(record.nome || "").trim().toLowerCase();
  var keyUfcd = String(ufcd || "").trim();
  var keyAno  = String(record.ano || cfg.ano || "").trim().toLowerCase();
  var updated = false;
  if (keyNome) {
    var lastRow = sh.getLastRow();
    if (lastRow >= 2) {
      var existing = sh.getRange(2, 1, lastRow - 1, hdrs.length).getValues();
      for (var i = 0; i < existing.length; i++) {
        var rNome = String(existing[i][1] || "").trim().toLowerCase();
        var rUfcd = String(existing[i][3] || "").trim();
        var rAno  = String(existing[i][4] || "").trim().toLowerCase();
        if (rNome === keyNome && rUfcd === keyUfcd && rAno === keyAno) {
          // Update in-place
  ...
  return {
    ...
    sheetName,
    updated: updated,
    msg: (updated ? "Atualizada: " : "") + `"${record.nome}" → ${sheetName} (${record.notaFinal}/20)`
  };
}
